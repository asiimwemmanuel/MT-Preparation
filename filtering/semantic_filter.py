...
def filter(
    ...
    with open(source_file_path, "r") as source_file, open(
        target_file_path, "r"
    ) as target_file, open(filtered_source, "a+") as filtered_source_file, open(
        filtered_target, "a+"
    ) as filtered_target_file:
        source = []
        target = []
        ...
        for src, tgt in zip(source_file, target_file):
            ...
            if line_index % chunk_size == 0 or line_index == file_line_count:
                ...
                source_embeddings = model.encode_multi_process(
                    source, pool=pool, batch_size=2048
                )
                target_embeddings = model.encode_multi_process(
                    target, pool=pool, batch_size=2048
                )

                # Find similar sentences (> threshold) and save to files
                index = 0

                for source_sentence_vecs, target_sentence_vecs in zip(
                    source_embeddings, target_embeddings
                ):
                    similarity = pytorch_cos_sim(
                        source_sentence_vecs, target_sentence_vecs
                    )
