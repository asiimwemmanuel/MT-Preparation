...
def prepare(source_file, target_file, source_lang, target_lang, lower=False):
    ...
    df["Too-Long"] = (
        ((df["Source"].str.count(" ") + 1) > (df["Target"].str.count(" ") + 1) * 2)
    ...
    df_source.to_csv(
        source_file, header=False, index=False, quoting=csv.QUOTE_NONE, sep="\n"
    )
    print("--- Source Saved:", source_file)
    df_target.to_csv(
        target_file, header=False, index=False, quoting=csv.QUOTE_NONE, sep="\n"
    )
